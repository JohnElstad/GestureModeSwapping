...
def follow(position):
    ...
    #positive x is left looking at front of rover, 0 is directly centered at the camera
    #Positie z is distance from camera. 0 is on camera



    # cmd_vel pub rules 
    x_thresh = .5 #meters from center of camera           
    follow_distance = 2
    z_thresh = .5
        ...
        if objects.is_new:
            ...
            if len(obj_array) > 0:
                ...
                if prediction == 0:
                    ...
                    linear_vel , angular_vel = follow(position)
