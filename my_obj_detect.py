        ...
        if zed.grab(runtime_params) == sl.ERROR_CODE.SUCCESS:
            ...
            if (returned_state == sl.ERROR_CODE.SUCCESS and objects.is_new):
                ...
                #    print("{} {}".format(object.id, object.position))
                    pos_x = object.position[0]
                    pos_y = object.position[1]
                    pos_z = object.position[2]
                ...
                agbot1_vel_msg = Twist()    
                pub_agbot1 = rospy.Publisher('AGBOT1_cmd_vel', Twist, queue_size=10)
                
                # cmd_vel pub rules 
                u_lx = 0.0; u_az = 0.0
                threshold_z = -2.35
                threshold_x = 0
                error_lx = threshold_z - pos_z
                error_az = threshold_x - pos_x            
                
                
                    
                if (abs(error_lx) <= 6):
                    u_lx = 5*error_lx
               # if (abs(error_lx) > 6) or (abs(u_az) > 0):
                 #   u_lx = 0
                if ((pos_x > -0.7) or (pos_x < -1.2)):
                    u_lx = 0
                ...
                agbot1_vel_msg.linear.y = pos_x
                agbot1_vel_msg.linear.z = pos_z
                
                
                
                agbot1_vel_msg.linear.x = u_lx
                agbot1_vel_msg.angular.z = u_az
                pub_agbot1.publish(agbot1_vel_msg)
...
def talker():
    ...
    rospy.init_node('ZED_obj_det', anonymous=True)
